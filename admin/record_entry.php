...
try {
    // Insert parking log with QR code - FIXED: Added qr_code to INSERT
    $insert_sql = "INSERT INTO parking_logs
                   (customer_id, qr_code, customer_name, plate, vehicle, entry_time, fee)
                   VALUES ($1, $2, $3, $4, $5, NOW(), 0.00)
                   RETURNING id";
    
    $log_id = db_insert_id($insert_sql, [
        $customer_id,
        $qr_code,  // Added QR code here
        $customer['name'],
        $customer['plate'],
        $customer['vehicle']
    ]);
    ...
    // Insert Arduino command (for gate control)
    try {
